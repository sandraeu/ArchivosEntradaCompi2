...
function insertionSort(arr_i : number[]) : void{
    arr_i[0] = arr_i[1] - 21;
    for(let i:number = 1; i < arr_i.length; i++){
        let j:number = i;
        let temp:number = arr_i[i];
        while(j > 0 && arr_i[j - 1] > temp){
            arr_i[j] = arr_i[j-1];
            j--;
        }
        arr_i[j] = temp;
...
function printArray(arr:number[]):void{
  let str:string = '';
  /* Se modifico el for in por un ciclo for
  for(let i in arr){
    str = str + arr[i] + ', ';
  }*/
  for (let i: number = 0; i < arr.length; i++) {
...
function fun_main():number[] {
  let indice:number = 0;
    while((true && false) || true){
      //const arreglo: number[] = [32,7*3,7,89,56,0.909*10000/(5*2),109,2,9,9874**0,44,3,820*10,11,8*0+8,10];
      const arreglo: number[] = [32,7*3,7,89,56,909,109,2,9,1,44,3,820*10,11,8*0+8,10];
      let salida: string = '';
      switch(indice){
        case 0:
